...
-/
private def char (s : Finset α) [DecidableEq α] : α →₀ ℕ where
  support := s
...
@[simp]
private theorem char_apply (s : Finset α) (a : α) [DecidableEq α] :
    char s a = if a ∈ s then 1 else 0 :=
  rfl
...
private theorem finsuppLex_of_toColex_lt_toColex
  ...
  have has : a ∉ s := by
    have hD := haD
    simpa [a, Finset.mem_symmDiff, hat] using hD
  rw [Finsupp.lex_def]
  refine ⟨a, ?_, ?_⟩
  · intro d hd
...
The data says that `t` is a term of `f`, the leading monomial of `p`
divides `t`, and the coefficient of the corresponding multiple of `p`
matches the coefficient of `f` at `t`. The result is obtained by subtracting
that multiple of `p` from `f`.
-/
def ReducesToBy
...
def TopReduces (p f g : MvPolynomial σ R) : Prop :=
...
theorem supportColexLt_wellFounded :
  ...
  letI : LinearOrder m.syn := m.lo
  haveI : WellFoundedLT m.syn := m.wf
  exact
    InvImage.wf
...
lemma coeff_eq_of_reduced_term_lt
  ...
  have hd_le_t : m.toSyn d ≤ m.toSyn t := by
    exact le_trans hd_le_prod (le_trans hmul (by simpa [hs] using hmon_add))
...
lemma mul_mem_reducesTo_subLTerm_mul
    ...
    (hfP : f ∈ P)
    (hLCf : IsUnit (m.leadingCoeff f))
    (hh : h ≠ 0) :
    (h * f) ⟶[m, P] m.subLTerm h * f := by
  have hlch : m.leadingCoeff h ≠ 0 := by
    simpa [MonomialOrder.leadingCoeff_eq_zero_iff] using hh
  have hreg : m.leadingCoeff f ∈ nonZeroDivisors R :=
    hLCf.mem_nonZeroDivisors
  have hf : f ≠ 0 := by
    intro hf0
    have hLCf0 : m.leadingCoeff f = 0 := by
      simpa [MonomialOrder.leadingCoeff_eq_zero_iff] using hf0
    have hzero : m.leadingCoeff f * m.leadingCoeff h = 0 := by
      simp [hLCf0]
    have hlch0 : m.leadingCoeff h = 0 :=
      (mul_left_mem_nonZeroDivisors_eq_zero_iff hreg).1 hzero
    exact hlch hlch0
  refine
...
/--
Lemma 5.24(i), unit-leading-coefficient version.

If `f ∈ P` and every nonzero element of `P` has unit leading coefficient,
then every polynomial multiple of `f` reduces to zero modulo `P`.
-/
theorem mul_mem_reflTransGen_zero
    ...
    (h * f) ⟶*[m, P] 0 := by
  have hP :
      ∀ p ∈ P, p ≠ 0 → IsUnit (m.leadingCoeff p) := by
    intro p hpP hp_ne
...
private lemma translation_step
  ...
  have hc_eq : c₁ - c₂ = c := by
    have hc' : c = ((u⁻¹ : Rˣ) : R) * h.coeff t := by
      rw [← hc, ← hu]
      simp only [mul_comm, mul_assoc, Units.mul_inv, mul_one]
    rw [hc']
    unfold c₁ c₂
    rw [← hc_diff]
    ring
...
theorem eqvGen_add_mul_mem
    ...
    f + h * p ⟷*[m, P] f := by
  have hred : h * p ⟶*[m, P] 0 :=
    mul_mem_reflTransGen_zero m P hP₀ hpP
    ...
    have hQ : Q (f - g) := by
      ...
      · intro a x hx hxQ h f
        have hx' :
            f + (h * a) * x ⟷*[m, P] f :=
          hxQ (h * a) f
        simpa [mul_assoc, smul_eq_mul] using hx'
...
theorem reflTransGen_singleton_to_reducesToPoly
  ...
  exact h.mono fun x y hxy =>
    (ReducesToSet.singleton_iff m).1 hxy
...
theorem reflTransGen_reducesToPoly_to_singleton
  ...
  exact h.mono fun x y hxy =>
    (ReducesToSet.singleton_iff m).2 hxy
...
theorem locallyConfluent_singleton
  ...
  simp only [Set.mem_singleton_iff] at hq₁P hq₂P
  subst q₁
  subst q₂
  ...
  have hf₁_eq' : f₁ = f - m₁ * p := by
    simpa [m₁] using hf₁_eq
...
theorem locallyConfluent_of_span_eq_singleton
  ...
  have hpLC : IsUnit (m.leadingCoeff p) := by
    rcases hP₀ p hpP with hunit | hp0
    · exact hunit
