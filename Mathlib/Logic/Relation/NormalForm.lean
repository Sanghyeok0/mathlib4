...
theorem confluent_of_wellFounded_flip_of_uniqueNormalForms
  ...
  have hEq : b' = c' :=
    hunf
      ...
      hb'.2
      hc'.2
  exact ⟨b', hb'.1, by simpa [hEq] using hc'.1⟩

/-- Newman's lemma: for a terminating relation, local confluence is equivalent to confluence. -/
theorem locallyConfluent_iff_confluent_of_wellFounded_flip
    (hwf : WellFounded (flip r)) :
    LocallyConfluent r ↔ Confluent r := by
  constructor
  · exact confluent_of_wellFounded_flip_of_locallyConfluent hwf
  · exact locallyConfluent_of_confluent

/-- For a terminating relation, confluence is equivalent to uniqueness of normal forms. -/
theorem confluent_iff_uniqueNormalForms_of_wellFounded_flip
    (hwf : WellFounded (flip r)) :
    Confluent r ↔ UniqueNormalForms r := by
  constructor
  · exact uniqueNormalForms_of_confluent
  · exact confluent_of_wellFounded_flip_of_uniqueNormalForms hwf

/-- For a terminating relation, uniqueness of normal forms is equivalent to the Church-Rosser
property. -/
theorem uniqueNormalForms_iff_churchRosser_of_wellFounded_flip
    (hwf : WellFounded (flip r)) :
    UniqueNormalForms r ↔ ChurchRosser r := by
  constructor
  · intro hunf
    exact Confluent.churchRosser (confluent_of_wellFounded_flip_of_uniqueNormalForms hwf hunf)
  · intro hcr
    exact Confluent.uniqueNormalForms (ChurchRosser.confluent hcr)
